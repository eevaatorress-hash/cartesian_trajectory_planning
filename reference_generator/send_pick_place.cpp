    ...
    double lambda)
{
    ...
    if (fabs(sin(theta / 2.0)) < 1e-6)
    {
        // No hay rotación significativa → eje no definido
        n = tf2::Vector3(0, 0, 0);  // o cualquier eje, no importa
    }
    else
    {
        n = tf2::Vector3(q_c.x(), q_c.y(), q_c.z()) / sin(theta / 2.0);
    }
    ...
    double t)
{
    ...
    else 
    {
        ...
        else    // Suavizado de la trayectoria cerca del punto intermedio
        {
            ...
            {
                n12 = tf2::Vector3(q12[0], q12[1], q12[2]) / sin(theta12 / 2.0);
...
int main(int argc, char **argv)
{
    ...
    Eigen::Matrix4d P_appro_pick;
    Eigen::Matrix4d P_pick;
    Eigen::Matrix4d P_appro_place;
